...
def weighted_loss(y_hat: List, y: Dict):
    weights = [1, 1, 1, 1, 1]

    if y["bowel"] == 1:
        weights[0] = 2

    if y["extravasation"] == 1:
        weights[1] = 6
    ...
    loss = weights[0] * (y_hat[0] != y["bowel"]) + \
        weights[1] * (y_hat[1] != y["extravasation"]) + \
        weights[2] * (y_hat[2] != y["kidney"]) + \
        weights[3] * (y_hat[3] != y["liver"]) + \
        weights[4] * (y_hat[4] != y["spleen"])
...
def train_step(model: torch.nn.Module,
    ...
    for batch, (X, y) in enumerate(dataloader):
        ...
        train_acc += (y == y_pred).sum().item() / len(y_pred)
...
def test_step(model: torch.nn.Module,
    ...
    with torch.inference_mode():
        ...
        for batch, (X, y) in enumerate(dataloader):
            ...
            # 2. Calculate and accumulate loss
            loss = weighted_loss(test_pred_logits, y)
            test_loss += loss.item()

            # Calculate and accumulate accuracy
            # test_pred_labels = test_pred_logits.argmax(dim = 1)
            # test_acc += ((test_pred_labels == y).sum().item() / len(test_pred_labels))
            y_pred = (test_pred_logits > 0.5).int()
            test_acc += (y == y_pred).sum().item() / len(y_pred)
